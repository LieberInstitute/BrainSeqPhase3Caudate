...
wgtlist = fread(opt$input,data.table=F)
...
genos = read_plink(paste0(opt$ref_ld_chr,chr),impute="avg")
...
if ( opt$plot ) {
    ...
    if ( !file.exists("glist-hg38") ) {
        if ( file.exists(paste0(script.dir,"/glist-hg38")) ){
            glist = fread(paste0(script.dir,"/glist-hg38"), data.table=F)
        ...
        glist = fread("glist-hg38", data.table=F)
        glist = glist[glist[,1] == chr,]
    }
...
if ( opt$omnibus ) {
    ...
    if ( is.na(opt$omnibus_corr) ) {
        cat( "GENE\tNUM.REF\tMIN.TWAS.P\tNUM.REF.PRUNED\tOMNIBUS.P\n", file=paste(opt$out, ".omnibus.pv", sep=''))
    }
    ...
    for ( g in dup.genes ) {
        ...
        if ( sum(cur.keep) > 1 ) {
            ...
            if ( !is.na(opt$omnibus_corr) ) {
                ...
                write.table( cbind(g,format(cur.tbl,digits=3)), quote=F, col.names=F, row.names=F,
                            sep='\t', file=paste( opt$out, ".CORR.dat", sep=''), append=T )	
            } else {
                ...
                if ( sum(!pruned) > 1 ) {
                    ...
                    cat( g, M, min( cur.wgt$TWAS.P ), sum(!pruned), pv.chi, '\n', sep='\t', 
                        file=paste(opt$out, ".omnibus.pv", sep=''), append=T )
                }
            }
        }
    }
...
if ( opt$ldsc ) {
    ...
    LDSC.df = data.frame( "CHR"=genos$bim[,1], SNP=genos$bim[,2], BP=genos$bim[,4], CM=genos$bim[,3], 
                         "MAF"=MAFS, "LD"=LDSC )
    write.table( LDSC.df, quote=F, row.names=F, col.names=T, file=f.LDSC )
    close(f.LDSC)
...
if( opt$zthresh ) {
    ...
    zthresh = qnorm( 0.10 / nrow( wgtlist ) / 2,lower.tail=F)
}

if( opt$verbose > 1 ) {
    cat( nrow( wgtlist ), " weights considered, a weight must have Z^2 > ", 
        zthresh^2, " to be retained in the model\n", sep='', file=stderr() )
}
...
if ( opt$plot_corr ) {
    ...
    par(cex = 0.3)
    corrplot( ge_g.ld^2, method="color", type="upper", order="hclust", addCoef.col="black", 
             tl.cex=0.5/0.3, col=colorRampPalette(c("blue","white","red"))(200),  cl.lim=c(0,1))
    par(cex = 1)
    dev.off()
}
...
if ( sum(cond.z^2 > zthresh^2) == 0 ) {
    if( opt$verbose > 0 ) {
        cat( "WARNING: no models had an absolute marginal association statistic higher than ", 
            zthresh, ". Skipping\n", sep='', file=stderr() )
    }
} else {
    while ( sum(cond.z^2 > zthresh^2) != 0 ) {
        ...
        if( opt$verbose > 1 ) {
            cat( wgtlist$FILE[ which.max(cond.z^2) ], " added to model with conditional Z-score of ", 
                cond.z[which.max(cond.z^2)], "\n", sep='', file=stderr() )
        }
        ...
        for ( i in 1:length(cond.z) ) {
            if ( ge.keep[i] || ge.drop[i] ) {
                ...
                if( opt$verbose > 1 ) {
                    cat( wgtlist$FILE[ i ],
                        " dropped from the model due to correlation higher than --max_r2 to other genes in the model\n", 
                        sep='', file=stderr() )
                }
            } else {
                ...
                if ( cur.b.var < 0 ) {
                    ...
                    if( opt$verbose > 1 ) {
                        cat( wgtlist$FILE[ i ], 
                            " dropped from the model due to misspecified correlation with other genes\n", 
                            sep='', file=stderr() )
                    }
        ...
        if ( sum(cur.unstable) != 0 ) {
            if( opt$verbose > 1 ) {
                cat( unlist( wgtlist$FILE[ cur.unstable ] ), 
                    " became more significant after conditional analysis and are dropped from the model (this is usually a sign of LD mismatch or a complex locus)\n", 
                    sep='', file=stderr() )
            }
    ...
    df.out = data.frame("FILE"=wgtlist$FILE[ge.keep], "ID"=wgtlist$ID[ge.keep], "TWAS.Z"=wgtlist$TWAS.Z[ge.keep],
                        "TWAS.P"=wgtlist$TWAS.P[ge.keep], "JOINT.BETA"=joint.b, "JOINT.BETA.SE"=joint.se, 
                        "JOINT.Z"=joint.z, "JOINT.P"=joint.p)
    write.table(df.out, quote=F, col.names=T, row.names=F, sep='\t', file=paste0(opt$out,".joint_included.dat") )
    ...
    df.out = data.frame("FILE"=wgtlist$FILE[!ge.keep], "ID"=wgtlist$ID[!ge.keep], "TWAS.Z"=wgtlist$TWAS.Z[!ge.keep], 
                        "TWAS.P"=wgtlist$TWAS.P[!ge.keep], "COND.BETA"=cond.b, "COND.BETA.SE"=cond.se, 
                        "COND.Z"=cond.z, "COND.P"=cond.p)
    write.table(df.out, quote=F, col.names=T, row.names=F, sep='\t', file=paste0(opt$out,".joint_dropped.dat") )
                                        # --- DONE FEATURE SELECTION
}

# --- PERFORM CONDITIONAL GWAS
# load in summary statistics and flip appropriately
sumstat = fread(opt$sumstats, data.table=F)
...
if ( opt$report ) {
    file.report = paste0(opt$out,".report")
...
for ( i in 1:length(cons.loc.starts) ) {
    ...
    if ( sum(cond.z^2 > zthresh^2) == 0 ) {
                ...
                if ( joint.keep[ii] ) {
                    ...
                    cur.b.var = ( 1 - ge_g.ld[ii,joint.keep,drop=F] %*% ( cur.dinv ) %*% t(ge_g.ld[ii,joint.keep,drop=F]))
                        if ( cur.b.var < 0 ) {
                            ...
                            cond.z[ii] = cur.b / sqrt( cur.b.var )
                        }
                }
            }
        }
        ...
        if ( opt$plot ) {
            ...
            pdf( file=paste0(opt$out,".loc_",i,".pdf"), height = 2 + log(tot)/2 )
            ...
            plot( 0, 0, type="n", ylim = c(-0.1,1.1), xlim = range( genos$bim[ cur.keep, 4 ] / 1e6 ), 
            ...
            for ( g in 1:nrow(cur.glist) ) {
                cur.row = 1
                while (sum(cur.gstart[g] - g.size < cur.gend & cur.gend[g] > cur.gstart - g.size & g.ypos == cur.row) != 0 ) {
                    cur.row = cur.row + 1
                }
                g.ypos[g] = cur.row
            }
            ...
            while ( TRUE ) {
                ...
                rect(cur.gstart, g.ypos - txt.scale*g.ysize*0.45, cur.gend, g.ypos + txt.scale*g.ysize*0.45,
                     border=clr.fg, col=clr.bg )
                ...
                if ( !is.na( opt$plot_legend) ) {
                    ...
                    legend("topleft", legend=paste(1:(length(uni.ref.names)),uni.ref.names), pch=19,
                           cex=txt.scale, col=clr.ref, bty="n")
                }
                                        # --- DONE GENE PLOTTING
                                        # Manhattan plots:
                if ( opt$plot_eqtl && ( sum(ge.keep) == 1 || g.ctr > 1 ) ) {
                    plot(genos$bim[ cur.keep, 4 ] / 1e6, -log10(pv), pch=19, cex=0.5, col="gray", xlab="",
                         ylab="-log10(P-value)", bty="n")
                } else {
                    plot( genos$bim[ cur.keep, 4 ] / 1e6, -log10(pv), pch=19, cex=0.5, col="gray",
                         xlab=paste0("chr ", chr, " physical position (MB)"), ylab="-log10(P-value)", bty="n" )
                }
                if ( g.ctr == 1 ) {
                ...
                if ( sum(ge.keep) == 1 || g.ctr > 1 ) {
                    if ( opt$plot_scatter ) {
                        plot(snp.ge.ld, snp.z , pch=19, cex=0.5, xlab="Corr. to TWAS", ylab="GWAS Z-score",
                             xlim=c(-1,1), ylim=c(-1*max(abs(snp.z)),max(abs(snp.z))))
                        par(xpd=F)
                        abline( 0, ge_g.z[cur.gene], lty=2 )
                        par(xpd=NA)					
                    } else if ( opt$plot_eqtl ) {
                        pv.eqtl = 2*(pnorm( abs(eqtl.z[[ cur.gene ]] ), lower.tail=F ))
                        plot(eqtl.pos[[cur.gene]] / 1e6, -log10(pv.eqtl), xlim=range( genos$bim[ cur.keep, 4 ] / 1e6 ),
                             pch=19, cex=0.5, col="#3182bd", xlab=paste0("chr ", chr, " physical position (MB)"),
                             ylab="-log10(P-value)", bty="n")
                    } 
                }
        ...
        if ( opt$save_loci ) {
            df.out = data.frame("SNP"=genos$bim[cur.keep,2], "POS"=genos$bim[cur.keep,4],
                                "GWAS.Z"=snp.z, "GWAS.P"=pv, "GWAS_cond.Z"=snp.cond.z, "GWAS_cond.P"=pv.cond )
            write.table(df.out, quote=F, col.names=T, row.names=F, sep='\t', file=paste0(opt$out,".loc_",i,".cond"))
        }
        if ( opt$report ) {
            df.out = data.frame("SNP"=genos$bim[cur.keep,2], "POS"=genos$bim[cur.keep,4],
                                "GWAS.LOGP"=-log10(pv), "GWAS_cond.LOGP"=-log10(pv.cond))
            write.table(df.out, quote=F, col.names=T, row.names=F, sep=',', file=paste0(opt$out,".loc_",i,".cond.csv"))
        }
        ...
        if ( opt$report ) {
            ...
            cat(paste(opt$out,".loc_",i,sep=''), opt$chr, range( genos$bim[ cur.keep, 4 ] ),
                length(unique(wgtlist$ID[ge.keep])), sum( joint.keep[ ge.keep ] ),
                min(wgtlist$TWAS.P[ ge.keep ],na.rm=T), 2*pnorm(sqrt(best.snp.chisq), lower.tail=F), 
                2*pnorm(sqrt(cond.snp.chisq), lower.tail=F), 1 - cond.snp.chisq / best.snp.chisq, '\n',
            ...
            write.table(cur.wgt, quote=F, row.names=F, col.names=T, sep='\t', file=paste0(opt$out,".loc_",i,".genes"))
        }
    }
}

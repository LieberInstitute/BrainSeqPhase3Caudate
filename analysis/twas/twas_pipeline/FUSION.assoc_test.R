...
sumstat = fread(opt$sumstats, data.table=F) %>% distinct(SNP, .keep_all=TRUE)

# Load in list of weights
# TODO : TEST FOR NO HEADER HERE
wgtlist = fread(opt$weights, data.table=F) %>% filter(CHR == opt$chr)
chr = unique(wgtlist$CHR)

N = nrow(wgtlist)
out.tbl = data.frame("PANEL"=rep(NA,N), "FILE"=character(N), "ID"=character(N), "CHR"=numeric(N),
                     "P0"=numeric(N), "P1"=numeric(N), "HSQ"=numeric(N), "BEST.GWAS.ID"=character(N),
                     "BEST.GWAS.Z"=numeric(N), "EQTL.ID"=character(N), "EQTL.R2"=numeric(N),
                     "EQTL.Z"=numeric(N), "EQTL.GWAS.Z"=numeric(N), "NSNP"=numeric(N), "NWGT"=numeric(N),
                     "MODEL"=character(N), "MODELCV.R2"=numeric(N), "MODELCV.PV"=numeric(N),
                     "TWAS.Z"=numeric(N), "TWAS.P"=numeric(N), stringsAsFactors=FALSE)
...
same.snps = intersect(genos$bim[,2], sumstat$SNP)
row.names(sumstat) = sumstat$SNP
sumstat = sumstat[same.snps, ]
row.names(genos$bim) = genos$bim[,2]
genos$bim = genos$bim[same.snps, ]
genos$bed = genos$bed[, same.snps]
...
for ( w in 1:nrow(wgtlist) ) {
                                        ...
                                        # Load weights
    wgt.file = paste0(opt$weights_dir,"/",wgtlist$WGT[w])
    load(wgt.file)
    ...
    if ( cur.FAIL || sum(eqtlmod) == 0 ) {
        ...
        if(dim(wgt.matrix)[1] == 1) {
            out.tbl$EQTL.ID[w] = rownames(wgt.matrix)
        } else {
    ...
    if ( !is.na(opt$coloc_P) && !is.na(out.tbl$TWAS.Z[w]) && out.tbl$TWAS.P[w] < opt$coloc_P && !is.na(wgtlist$N[w]) ) {
        ...
        err = suppressMessages(capture.output(clc <- coloc.abf(dataset1=list(beta=b1,varbeta=vb1,type="quant",
                                                                             N=wgtlist$N[w],sdY=1),
                                                               dataset2=list(beta=b2,varbeta=vb2,type="quant",
                                                                             N=opt$GWASN,sdY=1))))
...
if ( sum( mhc ) > 0 ) {
	...
	write.table( out.tbl[mhc,], quote=F, row.names=F, sep='\t', file=paste(opt$out,".MHC",sep='') )
}
write.table( out.tbl[!mhc,], quote=F, row.names=F, sep='\t', file=opt$out )

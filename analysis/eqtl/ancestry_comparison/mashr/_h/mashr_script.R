...
save_results <- function(pval, m2, feature){
    fn = paste0(feature,"/significant_geneSNP_pairs_ancestry.tsv")
    sig_eqtls <- get_significant_results(m2)
    df = get_n_significant_conditions(m2, thresh = 0.05, conditions = NULL,
                                      sig_fn = get_lfsr)
    aa = get_n_significant_conditions(m2, thresh = 0.05, conditions = 1,
                                      sig_fn = get_lfsr)
    ea = get_n_significant_conditions(m2, thresh = 0.05, conditions = 2,
                                      sig_fn = get_lfsr)
    eqtl = get_n_significant_conditions(m2, thresh = 0.05, conditions = 3,
                                        sig_fn = get_lfsr)
    dt = data.frame(N_Shared=df, AA=aa, EA=ea, ALL=eqtl) %>%
...
save_results_chunk <- function(m, bhat, shat, Vhat, feature, chunk, chunk_size){
    fn = paste0(feature,"/lfsr_allpairs_ancestry_",
                stringr::str_pad(chunk, 3, "left", pad="0"),".txt.gz")
    chunks = split(1:dim(bhat)[1],
                   cut(1:dim(bhat)[1], chunk_size, labels=FALSE))
    bhat.chunk = bhat[chunks[[chunk]],]
    shat.chunk = shat[chunks[[chunk]],]
    rm(bhat, shat, chunks)
    data.chunk = mash_set_data(bhat.chunk, shat.chunk, V=Vhat)
    ...
    data.frame(m.chunk$result$lfsr) %>%
        ...
        tidyr::separate(effect, c("gene_id", "variant_id"),
                        remove=FALSE, sep="_") %>%
        data.table::fwrite(fn, sep='\t', col.names=FALSE)
}
...
run_mashr <- function(feature, percentage, SEED){
    ...
    U.pca = cov_pca(data.strong, 3) # based on N conditions
...
parser$add_argument("-c", "--chunk", type="integer", help="Chunk to run")
...
if(!args$run_chunk){
    ...
    load(paste0(args$feature, "/chunk.RData"))
    save_results_chunk(m, bhat, shat, Vhat, args$feature,

...
def get_data(feature):
    ...
    aa = df[(df["AA"] < 0.05) & (df["EA"] >= 0.05) & (df["ALL"] >= 0.05)].copy()
    ea = df[(df["AA"] >= 0.05) & (df["EA"] < 0.05) & (df["ALL"] >= 0.05)].copy()
    eqtl = df[(df["AA"] >= 0.05) & (df["EA"] >= 0.05) &
             (df["ALL"] < 0.05)].copy()
    shared_df = df[(df["AA"] < 0.05) & (df["EA"] < 0.05) &
                   (df["ALL"] < 0.05)].copy()
    all_df = df[(df["AA"] < 0.05) | (df["EA"] < 0.05) |
                (df["ALL"] < 0.05)].copy()
    return aa, ea, eqtl, shared_df, all_df


def feature_summary(feature):
    aa, ea, eqtl, shared_df, all_df = get_data(feature)
    print(feature.upper())
    print("There are %d AA specific SNP-feature!" % aa.shape[0])
    print("There are %d EA specific SNP-feature!" % ea.shape[0])
    print("There are %d ALL specific SNP-feature!" % eqtl.shape[0])
...
def efeature_summary(feature):
    ...
    eqtl = eqtl.groupby("gene_id").first().reset_index()

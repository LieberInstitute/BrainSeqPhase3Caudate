...
@functools.lru_cache()
def load_eqtl():
    fn = "/ceph/projects/v4_phase3_paper/analysis/eqtl_analysis/all/genes/"+\
        "expression_gct/prepare_expression/annotate_outputs/_m/"+\
        "Brainseq_LIBD.signifpairs.txt.gz"
    return pd.read_csv(fn, sep='\t', usecols=[0,1,7])\
             .rename(columns={"slope": "all"})
...
def merge_data():
    eqtl = load_eqtl()
    aa, ea, sig = prepare_data()
    aa = eqtl.merge(aa, on=["gene_id", "variant_id"]).drop(["all"], axis=1)
    ea = eqtl.merge(ea, on=["gene_id", "variant_id"]).drop(["all"], axis=1)
    sig = eqtl.merge(sig, on=["gene_id", "variant_id"]).drop(["all"], axis=1)
    return pd.merge(aa, ea, on=["gene_id", "variant_id"])\
             .merge(sig, on=["gene_id", "variant_id"])
...
def plotNsave_corr(df):
    ...
    ggplot2::ggsave(plot=figure, dpi=72, width=9, height=3,
                    filename="scatter_slope_ancestry_eqtl.pdf")
    ggplot2::ggsave(plot=figure, dpi=300, width=9, height=3,
                    filename="scatter_slope_ancestry_eqtl.png")
    ''')

...
def merge_data():
    ...
    ctl = eqtl.merge(ctl, on=["gene_id", "variant_id"]).drop(["all"], axis=1)
    szAP = eqtl.merge(szAP, on=["gene_id", "variant_id"]).drop(["all"], axis=1)
    noAP = eqtl.merge(noAP, on=["gene_id", "variant_id"]).drop(["all"], axis=1)
...
def plotNsave_corr(df):
    ...
    ggplot2::ggsave(plot=figure, dpi=72, width=9, height=3,
                    filename="scatter_slope_antipsychotics_eqtl.pdf")
    ggplot2::ggsave(plot=figure, dpi=300, width=9, height=3,
                    filename="scatter_slope_antipsychotics_eqtl.png")
    ''')
